...
# Setup path agar bisa import dari folder lain
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(BASE_DIR)

# Import komponen-komponen proyek
    ...
    def __init__(self):
        ...
        for index, row in self.COURSE_DATA.iterrows():
            ...
            if len(initials) == len(clean_names_list):
                ...
                print(f"[PERINGATAN DATA] Baris {index + 2} ({row['KodeMatkul']}): Inisial & nama tidak cocok. Dosen dari baris ini tidak akan dimuat.")
...
class Schedule:
        ...
        for i, course in enumerate(courses):
            ...
            if course.is_fixed:
                for schedule_str in course.fixed_schedules:
                    ...
                    else: print(f"PERINGATAN: Jadwal tetap '{schedule_str}' untuk {course.number} tidak ditemukan.")
            else:
    ...
    def check_time_overlap(self, c1, c2):
        ...
        for mt in c1.meeting_times: c1_slots.update(time_mapper.get_1hr_slots(mt.id))
        for mt in c2.meeting_times: c2_slots.update(time_mapper.get_1hr_slots(mt.id))
    ...
    def calculate_fitness_and_score(self):
        ...
        def _parse_simple(time_str):
            try:
                day, times = time_str.split('-', 1)
                start_hour = int(times.split('.')[0])
        ...
        for c in self._classes:
            ...
            if constraints_loader.is_enabled('K8') and len(c.meeting_times) > 1:
            ...
            for meeting_time in c.meeting_times:
                if constraints_loader.is_enabled('K6') and meeting_time.is_blocked: hard_conflicts += 1
        ...
        for i in range(len(parent2_genes)):
            if random.random() < settings.CROSSOVER_RATE:
                ...
                if gene_from_parent2.id != gene_in_child_at_pos_i.id:
                    ...
                    child_genes_list[original_pos] = gene_in_child_at_pos_i
                    child_gene_map[gene_from_parent2.id] = i
                    child_gene_map[gene_in_child_at_pos_i.id] = original_pos
        ...
        for i in range(len(schedule.get_classes())):
            ...
            if current_class.meeting_times and random.random() < settings.MUTATION_RATE:
                ...
                if possible_new_times and len(possible_new_times) > 1:
                    ...
                    while new_meeting_time.id == session_to_mutate.id:
                        new_meeting_time = random.choice(possible_new_times)
                    try:
...
class DisplayManager:
    ...
    def print_schedule_as_table(self, schedule, generation_number, elapsed_time):
        ...
        output_filename = "Jadwal_Final_Optimal6.csv"
        final_path = os.path.join(BASE_DIR, output_filename)
        df.to_csv(final_path, index=False)
        print(f"\nJadwal optimal telah disimpan ke file '{final_path}'")

if __name__ == '__main__':
    # Pastikan 'import sys' dan 'import copy' ada di bagian paling atas file Anda
    ...
    best_schedule_overall = population.get_schedules()[0]
    initial_best_score = best_schedule_overall.get_score()
    
    # === PERBAIKAN KUNCI DI SINI ===
    # Jika solusi sempurna sudah ada di awal, tampilkan dan keluar.
    if initial_best_score == 0:
        print(f"\nJadwal Awal Terbaik -> Skor: {initial_best_score} (H: {best_schedule_overall.get_hard_conflicts()}, S: {best_schedule_overall.get_soft_conflicts()})")
        print("\n\n--- HASIL AKHIR EVOLUSI ---")
        print("Solusi Sempurna (Skor = 0) ditemukan di populasi awal! Tidak perlu evolusi.")
        
        # Langsung tampilkan hasilnya
        display_manager.print_schedule_as_table(best_schedule_overall, 0, time.time() - start_time)
        
        # Keluar dari program
        sys.exit()

    # Kode di bawah ini hanya akan berjalan jika solusi awal TIDAK sempurna
    print(f"Jadwal Awal Terbaik -> Skor: {initial_best_score} (H: {best_schedule_overall.get_hard_conflicts()}, S: {best_schedule_overall.get_soft_conflicts()})")
    print(f"\nMemulai proses evolusi untuk MAKSIMAL {settings.MAX_GENERATION} generasi...")
    ...
    with tqdm(total=settings.MAX_GENERATION, desc="Evolusi Generasi") as pbar:
        for i in range(settings.MAX_GENERATION):
            generation_num = i + 1
            population = genetic_algorithm.evolve(population)
            population.get_schedules().sort(key=lambda x: x.get_fitness(), reverse=True)
            
            current_best_in_generation = population.get_schedules()[0]
            
            if current_best_in_generation.get_score() < best_schedule_overall.get_score():
                best_schedule_overall = copy.deepcopy(current_best_in_generation)
            ...
            if best_schedule_overall.get_score() == 0:
                pbar.n = settings.MAX_GENERATION
                pbar.refresh()
                print(f"\n\nSolusi Sempurna (Skor = 0) ditemukan pada generasi ke-{generation_num}!")
                break

            generations_without_improvement = generation_num - last_improvement_generation
            if generations_without_improvement >= settings.EARLY_STOPPING_PATIENCE:
                pbar.n = settings.MAX_GENERATION
                pbar.refresh()
                print(f"\n\nEarly stopping tercapai pada generasi ke-{generation_num} karena stagnasi.")
                print(f"   -> Skor terbaik tidak membaik selama {settings.EARLY_STOPPING_PATIENCE} generasi.")
                break
